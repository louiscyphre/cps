...
public class TestInsertionAlgorithm extends ServerControllerTest {
  ...
  public void testInsertCars() throws ServerException {
    ...
    a[0] = Timestamp.valueOf(getTime().plusMinutes(5));
    // exit in two hours
    a[1] = Timestamp.valueOf(getTime().plusHours(2));
    // exit in seven hours
    a[2] = Timestamp.valueOf(getTime().plusHours(7));
    // exit in 13 hours
    a[3] = Timestamp.valueOf(getTime().plusHours(13));
    // Exit in 18 hours
    a[4] = Timestamp.valueOf(getTime().plusHours(18));
    // Exit in 24 hours
    a[5] = Timestamp.valueOf(getTime().plusHours(24));
    // Exit in 26 hours
    a[6] = Timestamp.valueOf(getTime().plusHours(26));
    // Exit in 30 hours
    a[7] = Timestamp.valueOf(getTime().plusHours(30));
    // Exit in 40 hours
    a[8] = Timestamp.valueOf(getTime().plusHours(40));
    // Exit in 50 hours
    a[9] = Timestamp.valueOf(getTime().plusHours(50));
...

    {
      for (int j = 0; j < carsInLot1; j++) {
        ...
        transcontroller.insertCar(conn, lot[0], reservedParkings1[j].getCarID(), reservedParkings1[j].getExitTime(getTime().toLocalDate()));
      }
    });
...

    {
      for (int j = 0; j < carsInLot2; j++) {
        ...
        transcontroller.insertCar(conn, lot[1], reservedParkings2[j].getCarID(), reservedParkings2[j].getExitTime(getTime().toLocalDate()));
      }
    });
  ...
  public void testOverlap() throws ServerException {
    ...
    a[0] = Timestamp.valueOf(getTime().plusMinutes(10));
    ...
    a[3] = Timestamp.valueOf(getTime().plusMinutes(30));
    // Reservation that doesn't overlap start
    a[4] = Timestamp.valueOf(getTime().plusMinutes(5));
  ...
  private CarTransportation newParkingEntry(OnetimeService _cdata) {
    ...
    return new CarTransportation(respo.getCustomerID(), _cdata.getCarID(), Constants.LICENSE_TYPE_ONETIME,
        _cdata.getId(), _cdata.getLotID(), Timestamp.valueOf(getTime()), null);
  }
