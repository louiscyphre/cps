...
public class CarTransportationController1 extends RequestController implements CarTransportationController {
  public CarTransportationController1(ServerController serverController) {
  ...
  private boolean insertCars(Connection conn, ParkingLot lot, Stack<String> carIds, Stack<LocalDateTime> exitTimes)
    ...
    while (!carIds.isEmpty()) {
      ...
      /*
       * Lower number represents higher priority - the car will be closer to
       * exit 0<-->4
       */
      priority = 0;
      ...
      /*
       * Calculate exit priority if there is no exit time - it means we deal
       * with FULL SUBSCRIPTION which will perhaps stay for long hours or even
       * more than one day
       */
      if (exitTime == null || exitTime.equals(fullSubscriptionTime)) {
        ...
        /*
         * divide difference between now and exit time by the time left today
      ...
      while (maxSize == -1) {
        switch (priority) {
          case 0:
            ...
            /*
             * If we reached end of the line and have not found a place for this
             * priority, lower the priority and try again
             */
            if (maxSize == -1) {
              priority++;
            }
